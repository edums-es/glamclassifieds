...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      // TanStack Start v1 SSR leaks async_hooks into the client bundle on dev.
      // Map it to a browser-safe stub so Vite can bundle the client entry.
  ...
  optimizeDeps: {
    ...
    // DYNAMIC import. That entry statically imports react, react-dom/client and
    // @tanstack/react-start/client (→ @tanstack/react-router). If any of those is
    // NOT already optimized, the first post-build page load DISCOVERS it and kicks
    // off an on-demand dep re-optimize — and the entry's in-flight dynamic import
    // can land mid-optimize and fail with "Failed to fetch dynamically imported
    // module: …/default-entry/client.tsx", so a freshly-built site shows a BLANK
    // preview (the dep chunks 504 while GET / still 200s → invisible to health
    // probes; #1 preview error, ~2.9k projects). Listing the entry's whole closure
    // here optimizes it ONCE at boot, so loading the entry never triggers (or
    // races) a runtime re-optimize. Dev-only — optimizeDeps does NOT touch the
    // production / prerender build, so SSR + SEO are unchanged.
    include: [
      'react',
      'react-dom',
      'react-dom/client',
