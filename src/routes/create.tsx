...
function CreateProfilePage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      await profilesApi.submit({ name, age, category, city, neighborhood, price, contactPhone, availability, description, tags: selectedTags, photos: photos.map(photo => photo.file), adultConfirmed })
      setSubmitted(true)
    ...
    <main className="min-h-dvh bg-[#fff8fb] pb-16">
      ...
      <section className="mx-auto grid max-w-6xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:py-14">
        ...
        <form onSubmit={submit} className="model-form rounded-3xl border border-pink-100 bg-white p-6 shadow-sm sm:p-9">
          <div><p className="text-sm font-bold uppercase tracking-[0.16em] text-pink-600">Etapa única</p><h2 className="mt-2 text-2xl font-black text-slate-900">Cadastre seu perfil</h2><p className="mt-2 text-sm text-slate-500">Campos com <strong>*</strong> são obrigatórios.</p></div>
